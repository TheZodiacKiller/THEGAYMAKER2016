...
//Q ABILITY
if (q_key) && o_player_stats.mp >= 100 && o_player_stats.q_cooldown <= 0 {
    image_index = 0;
    state = mageicewave;
    with(o_player_stats) {
        mp -= 100;
...
//E ABILITY
if (e_key) && o_player_stats.mp >= 90 && o_player_stats.e_cooldown <= 0 {
    instance_create(x,y,fireiceeffect);
    instance_create(x,y,o_fireicebuff);
    with(o_player_stats) {
        mp -= 90;

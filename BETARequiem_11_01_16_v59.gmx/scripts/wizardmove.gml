...
//Vertical Sprites
if (vspd > 0) {
sprite_index = spr_wizardDown;
} else if (vspd < 0) {
sprite_index = spr_wizardUp;
}

//Horizontal Sprites
if (hspd > 0) {
    sprite_index = spr_wizardRight;
} else if (hspd < 0) {
    sprite_index = spr_wizardLeft;
}
...
//RC ABILITY
if (rc_key) && o_player_stats.mp >= 80 && o_player_stats.rc_cooldown <= 0 {
    image_index = 0;
    state = wizardglacier;
    with(o_player_stats) {
        mp -= 80;

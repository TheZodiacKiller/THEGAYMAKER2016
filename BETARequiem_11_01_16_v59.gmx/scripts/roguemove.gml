...
//Vertical Sprites
if (vspd > 0) {
sprite_index = spr_rogueDown;
} else if (vspd < 0) {
sprite_index = spr_rogueUp;
}

//Horizontal Sprites
if (hspd > 0) {
    sprite_index = spr_rogueRight;
} else if (hspd < 0) {
    sprite_index = spr_rogueLeft;
}
...
//RC
if (rc_key) && o_player_stats.mp >= 30 && o_player_stats.rc_cooldown <= 0 {
    image_index = 0;
...
//Q
if (q_key) && o_player_stats.mp >= 50 && o_player_stats.q_cooldown <= 0 {
    image_index = 0;
    state = roguevitalstrike;
    with(o_player_stats) {
        mp -= 50;
...
//E
if (e_key) && o_player_stats.mp >= 60 && o_player_stats.e_cooldown <= 0 {
    image_index = 0;
    state = roguefade;
    with(o_player_stats) {
        mp -= 60;

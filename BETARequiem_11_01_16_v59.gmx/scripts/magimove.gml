...
//Vertical Sprites
if (vspd > 0) {
sprite_index = spr_magiDown;
} else if (vspd < 0) {
sprite_index = spr_magiUp;
}

//Horizontal Sprites
if (hspd > 0) {
    sprite_index = spr_magiRight;
} else if (hspd < 0) {
    sprite_index = spr_magiLeft;
}
...
//RC ABILITY
if (rc_key) && o_player_stats.mp >= 100 && o_player_stats.rc_cooldown <= 0 {
    image_index = 0;
    state = magipillarofflame;
    with(o_player_stats) {
        mp -= 150;
...
//Q ABILITY
if (q_key) && o_player_stats.mp >= 160 && o_player_stats.q_cooldown <= 0 {
    image_index = 0;
    state = magiflameconduit;
    with(o_player_stats) {
        mp -= 160;
...
//E ABILITY
if (e_key) && o_player_stats.mp >= 120 && o_player_stats.e_cooldown <= 0 {
    image_index = 0;
    state = magifireelemental;
    with(o_player_stats) {
        mp -= 120;
